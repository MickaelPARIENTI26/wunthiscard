...
| 2026-09-02 | Correction juridique posée sur `/api/tickets/free-entry` (route web) alors que la vraie voie postale est `assignFreeEntry` côté admin — la route web refuse tout concours payant, donc le correctif ratait exactement les concours concernés | Avant de corriger « la voie gratuite », lister TOUTES les routes qui créent un ticket `FREE_ENTRY` (`grep -rn "status: 'FREE_ENTRY'"`) : il y en a deux, dans deux apps |
