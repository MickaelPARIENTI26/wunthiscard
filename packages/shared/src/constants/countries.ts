// Single source of truth for country pickers (address forms + phone country-code
// selectors) across the app. Previously each form (guest checkout, add-address,
// edit-address) hardcoded its own ~18-22 country shortlist, none of which
// included most of Asia, the Middle East, Africa or South America — e.g. a user
// couldn't select Israel at all. Sovereign states (UN member states) plus a
// handful of commonly-needed territories (Hong Kong, Taiwan, Kosovo, Palestine).
export interface Country {
  code: string; // ISO 3166-1 alpha-2
  name: string;
  phoneCode: string; // E.164 calling code, e.g. "+44"
}

// United Kingdom first (primary market / form default), then the rest of the
// world alphabetically by English name.
...
// Flag emoji computed from the ISO code (regional indicator symbols) rather than
// stored per-row — guarantees correctness for all ~195 entries with no per-country
// typo risk.
